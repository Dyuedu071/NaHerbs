...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/san-pham/${product.slug}`} className="bg-surface-container-lowest rounded-xl p-sm shadow-[0_4px_20px_-2px_rgba(46,77,57,0.12)] group block h-full flex flex-col transition-shadow hover:shadow-[0_8px_30px_-4px_rgba(46,77,57,0.2)]">
      <div className="relative mb-sm overflow-hidden rounded-xl bg-surface-container-low aspect-square flex-shrink-0">
        ...
        {/* Status Badge */}
        {isOutOfStock ? (
          <span className="absolute top-3 left-3 px-3 py-1 rounded-full bg-surface-container-high text-text-muted font-label-md text-[12px]">Hết hàng</span>
        ) : (
          <span className="absolute top-3 left-3 px-3 py-1 rounded-full bg-success-bg text-primary font-label-md text-[12px]">Còn hàng</span>
        )}
